...
{
		now.sync_P_Q = addqueue(calling_pid, 1, 1);
		now.sync_P_R = addqueue(calling_pid, 2, 1);
		now.sync_R_Q = addqueue(calling_pid, 3, 1);
#ifdef VAR_RANGES
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.sync_P_Q) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.sync_P_Q) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.sync_P_Q) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.sync_P_Q) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.sync_P_R) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.sync_P_R) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.sync_P_R) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.sync_P_R) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.sync_R_Q) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.sync_R_Q) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.sync_R_Q) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.sync_R_Q) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	chan sync_P_Q (=%d):	len %d:\t", now.sync_P_Q, q_len(now.sync_P_Q));
	c_chandump(now.sync_P_Q);
	printf("	chan sync_P_R (=%d):	len %d:\t", now.sync_P_R, q_len(now.sync_P_R));
	c_chandump(now.sync_P_R);
	printf("	chan sync_R_Q (=%d):	len %d:\t", now.sync_R_Q, q_len(now.sync_R_Q));
	c_chandump(now.sync_R_Q);
}
void
